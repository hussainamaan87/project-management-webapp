...
const Navbar = () => {
    ...
    <div className='flex items-center justify-between bg-white px-4 py-3 dark:bg-black'>
        ...
        <div className="flex items-center gap-8">
            {!isSidebarCollapsed ? null : (
                <button onClick={() => dispatch(setIsSidebarCollapsed(!isSidebarCollapsed))}>
                    <Menu className='h-8 w-8 dark:tex-white'></Menu>
                </button>
            )}
        ...
        <div className='flex items-center'>
            <button className={isDarkMode ? `rounded p-2 dark:hover:bg-gray-700` : `rounded p-2 dark:hover:bg-gray-100`} onClick={() => dispatch(setIsDarkMode(!isDarkMode))}>
                {isDarkMode? (<Sun className='h-6 w-6 cursor-pointer dark:text-white'/>):(<Moon className='h-6 w-6 cursor-pointer dark:text-white'/>)}
            </button>
            <Link href='/settings' className={isDarkMode ? `h-min w-min rounded p-2 dark:hover:bg-gray-700` : `h-min w-min rounded p-2 dark:hover:bg-gray-100`}>
